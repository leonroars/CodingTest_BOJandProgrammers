...
// 하지만 우선순위 큐는 삭제를 통해 현재 상태에서의 최대/최소값에 대한 효율적 접근을 제공한다.
// 이러한 방식으로 설계된 인접리스트를 DFS와 BFS가 공유할 수 없다.
// 물론 Synchronized를 제공하는 PriorityBlockingQueue와 멀티스레딩을 이용한다면 가능할지도 모르겠으나(어디까지나 아이디어일 뿐이다)
// 그렇지 않은 이상 순차적 처리를 요구하고 있기 때문이다.
// 메모리 제한이 128MB로 넉넉하지 않기 때문에 각각의 탐색 알고리즘을 위한 인접리스트를 따로 만드는 것도 어려울 것이다.
...
//    DFS/BFS 공유가 필요하지 않은 것들은 모두 로컬 변수로 생성 후 사용하도록하여 메서드 호출 종료 시 GC가 되도록 함으로써 메모리 부하를 덜어준다.
//    단, BitSet visited는 DFS와 BFS를 따로 관리한다. - DFSvisited, BFSvisited
//    이는 불필요한 정렬을 생략하기 위함이다.
//    이에 대해 조금 더 설명을 붙이자면
//    멀티스레딩을 하지 않는 한 두 탐색은 순차적으로 이루어지므로 둘 중 어떤 쪽이 먼저 처리될지 정해야한다.
